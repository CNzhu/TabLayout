...
public class TabLayout extends HorizontalScrollView {
    ...
    private int indictBottomOffset = 30;
    private int indictHorizonOffset = 30;

    public void setIndictHorizonOffset(int indictHorizonOffset) {
        this.indictHorizonOffset = indictHorizonOffset;
    }

    public void setIndictBottomOffset(int indictOffset) {
    ...
    private void addTabFromItemView(@NonNull TabItem item) {
        ...
        try {
            Field mTextField = item.getClass().getDeclaredField("mText");

            mTextField.setAccessible(true);
            CharSequence mText = (CharSequence) mTextField.get(item);

            Field drawableFiled = item.getClass().getDeclaredField("mIcon");

            drawableFiled.setAccessible(true);
            Drawable mIcon = (Drawable) drawableFiled.get(item);

            Field layoutFiled = item.getClass().getDeclaredField("mCustomLayout");

            layoutFiled.setAccessible(true);
            int layout = (int) layoutFiled.get(item);
